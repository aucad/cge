...
from comparison.moeva.moeva import Moeva2
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
                TensorflowClassifier(
                    load_model(
                        "./comparison/resources/baseline_nn.model")
                )
        ),
    ],
)
...
def test_objective_calculation():
    constraints = get_url_constraints()
    x_clean = np.load(
        "./comparison/resources/baseline_X_test_candidates.npy")[:10]
    y_clean = np.load(
        "./comparison/resources/baseline_y_test_candidates.npy")[:10]
    model = joblib.load("./comparison/resources/baseline_rf.model")
    preprocessing_pipeline = joblib.load(
        "./comparison/resources/baseline_scaler.joblib"
    )
