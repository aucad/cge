...
from comparison import EPS, NumpyConstraintsExecutor, \
...
class TestRelationConstraint:
    ...
    def test_safe_division(self):
        ...
        result = Feature(2)
        constraint = SafeDivision(dividend, divisor,
                                  safe_value) == result
        executor = NumpyConstraintsExecutor(constraint)
        out = executor.execute(x)
    ...
    def test_less_constraint(self, left_operand, right_operand,
                             expected_out):
        left_operand = Constant(left_operand)
        right_operand = Constant(right_operand)
    ...
    def test_equal_constraint(self, left_operand, right_operand,
                              expected_out):
        left_operand = Constant(left_operand)
        right_operand = Constant(right_operand)
