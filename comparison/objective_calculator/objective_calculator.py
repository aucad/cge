...
class ObjectiveCalculator:
    ...
    def _calculate_objective(self, x_clean, y_clean, x_adv):
        ...
        constraint_violation = 1 - constraints_checker \
            .check_constraints(x_clean[np.newaxis, :], x_adv)
    ...
    def _objective_array(self, x_clean, y_clean, x_adv):
        objective_values = self._calculate_objective(x_clean, y_clean,
                                                     x_adv)
        return self._objective_respected(objective_values, y_clean)

    def success_rate(self, x_clean, y_clean, x_adv):
        return self._objective_array(x_clean, y_clean, x_adv).mean(
            axis=0)
    ...
    def success_rate_many(self, x_clean, y_clean, x_adv):
        at_least_one = np.array(
            [
                ...
                for i, x_adv_i in
                tqdm(enumerate(x_adv), total=len(x_adv))
            ]
        )
    ...
    def success_rate_many_df(self, x_clean, y_clean, x_ad):
        ...
        columns = ["o{}".format(i + 1) for i in
                   range(success_rates.shape[0])]
    ...
    def get_successful_attacks(
        ...
        if return_index_success:
            index_success = np.array(
                [len(e) >= 1 for e in successful_attacks])
