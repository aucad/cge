...
- **Attacks**: Projected Gradient Descent (PGD), Zeroth-Order Optimization (ZOO), HopSkipJump attack, modified to include our constraint validation algorithm.
...
A single experiment execution consists of training a classification model, 
trained on one of the input data sets, and then applying an adversarial attack on 
that model. The validation algorithm can be enabled or disabled during the attack,
to impact the validity of the generated adversarial examples.
